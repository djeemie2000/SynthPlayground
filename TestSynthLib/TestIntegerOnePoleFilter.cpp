...
TEST(LowPassIncrease2)
{
    ...
    const int Parameter = 128;//0.5f;
    Filter.SetParameter(Parameter);
    CHECK_CLOSE(2048, Filter(0), 0.5f);
    CHECK_CLOSE(2048, Filter(0), 0.5f);
    CHECK_CLOSE(2048, Filter(0), 0.5f);
...
TEST(LowPassIncreaseDecrease)
{
    ...
    CHECK_CLOSE(2048+2048*0.81f, Filter(4096), 0.5f);
    CHECK_CLOSE(2048+2048*0.9639f, Filter(4096), 0.5f);
    CHECK_CLOSE(2048+2048*0.993141f, Filter(4096), 0.5f);

    CHECK_CLOSE(2048+2048*0.188697f, Filter(2048), 0.5f);
    CHECK_CLOSE(2048+2048*0.0358524f, Filter(2048), 0.5f);
    CHECK_CLOSE(2048+2048*0.00681196f, Filter(2048), 0.5f);
}
...
TEST(LowPassIncreaseDecrease2)
{
    ...
    CHECK_CLOSE(2048*0.81f, Filter(2048), 0.5f);
    CHECK_CLOSE(2048*0.9639f, Filter(2048), 0.5f);
    CHECK_CLOSE(2048*0.993141f, Filter(2048), 0.5f);

    CHECK_CLOSE(2048*0.188697f, Filter(0), 0.5f);
    CHECK_CLOSE(2048*0.0358524f, Filter(0), 0.5f);
    CHECK_CLOSE(2048*0.00681196f, Filter(0), 0.5f);
}
